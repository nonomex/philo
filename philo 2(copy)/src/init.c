...
/*   Updated: 2023/11/09 14:30:03 by aammirat         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	initfork(t_table	*table)
{
	...
	while (i < table->nb_philo)
	{
		...
		{
			table->philo[i].forkrightlock = &table->islock[i + 1];
...
int	inithuman(t_table *table)
{
	...
	while (++i < table->nb_philo)
	{
		...
		if (pthread_mutex_init(&table->philo[i].number, NULL) != 0)
			return (0);
		table->whereinit++;
		table->philo[i].lastime = (pthread_mutex_t)PTHREAD_MUTEX_INITIALIZER;
		if (pthread_mutex_init(&table->philo[i].lastime, NULL) != 0)
			return (0);
		table->whereinit++;
	}
